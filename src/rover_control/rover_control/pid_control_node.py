...
class PIDControlNode(Node):
    def __init__(self):
        ...
        self.target_speed = 4000  # Setpoint for target speed
    ...
    def listener_callback(self, msg):
        ...
        # # Convert control signal to speed values
        # speed_control = min(max(int(control), 0), 255)
        # speed_control2 = min(max(int(control2), 0), 255)
        ...
        # Log the results
        self.get_logger().info(f'Motor1 Speed: {speed} Control: {control}')
        self.get_logger().info(f'Motor2 Speed: {speed2} Control: {control2}')
